...
namespace Kakera
{
    public class PickerControllerAll3 : MonoBehaviour
	{
		[SerializeField]
		...
		//下記、テクスチャを正確に貼り替えるために修正したスクリプトを追記
		IEnumerator ShowCoroutine(){
			//レンダラーのマテリアルの数だけループする
			for (int i = 0; i < imageRenderer.materials.Length; i++) {
				//選択画面表示
				imagePicker.Show("Select Image", "unimgpicker", 1024);
				//１秒待つ：テクスチャが張り替えられるのを待つ
				yield return new WaitForSeconds (1);
				//張り替え終わったらindexを更新する
				index++;
			}
			//全て張り替え終わったらindexをリセットする
			index = 0;
		}
		...
		private IEnumerator LoadImage(string path, MeshRenderer output)
		{
			...
			//下記2回目の設定値
			//index++; //IEnumerator ShowCoroutine()を追加時に削除
